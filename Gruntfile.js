module.exports = function (grunt) {
    ...
    grunt.registerTask('public-deploy', function () {
        ...
        args.push(name, gitHubURL);

        var bowerPublish = grunt.util.spawn({
            cmd: 'bower',
            args: args,
            opts: {
                cwd: '.'
        ...
        if (typeof bowerPublish === 'undefined') {
            grunt.fail.fatal('Bower publish task failed.');
        }
        bowerPublish.stdout.on('data', function (buf) {
            grunt.log.write(String(buf));
        });
        bowerPublish.stderr.on('data', function (buf) {
